...
typedef struct tCHS // 20 bits
{
	unsigned char sector; // 6 bits
	unsigned char head; // 4 bits
	unsigned short int cylinder; // 10 bits
};

typedef struct tLBA // 32 bits 
{
	unsigned int size;
};

typedef struct tLARGE // 24 bits
{
	unsigned char sector; // 6 bits
	unsigned char head; // 8 bits
	unsigned short int cylinder; // 10 bits
};

typedef struct tIDECHS // 28 bites
{
